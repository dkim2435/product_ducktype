...
export function ShareButton({ result, onShareClick, fullWidth }: ShareButtonProps) {
  ...
  const btnClass = "flex items-center gap-[10px] w-full p-[10px_14px] text-[13px] text-text text-left rounded-[6px] transition-colors duration-100 hover:bg-bg";
  ...
  return (
    <div ref={menuRef} className="relative">
      ...
      <button
        ...
        className={`p-[10px_24px] text-sm text-bg bg-main rounded-default flex items-center justify-center gap-2 font-semibold${fullWidth ? ' w-full' : ''}`}
      >
        {/* Share icon */}
      ...
      {menuOpen && (
        <div
          className="fade-in flex flex-col gap-[2px] bg-sub-alt rounded-default p-1.5 z-[100]"
          style={isMobile ? {
            position: 'fixed',
            top: '50%',
            left: '50%',
        ...
        >
          ...
          <button onClick={handleKakao} className={btnClass}>
